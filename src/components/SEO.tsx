...
export const SEO: React.FC<SEOProps> = ({ data }) => {
  useEffect(() => {
    const siteUrl = 'https://plansimba.com';
    ...
    const image = data.og_image?.url ? `https://mypowerly.com/v1${data.og_image.url}` : `${siteUrl}/partner.png`;
    ...
    const orgSchema = {
      ...
      contactPoint: {
        ...
        email: data.footer_config?.contact_info?.email || 'support@plansimba.com',
        areaServed: 'US',
